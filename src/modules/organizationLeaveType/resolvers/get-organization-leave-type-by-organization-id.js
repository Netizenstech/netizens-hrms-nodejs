...
const getOrganizationPolicyByOrganizationId = async (_, args, { models }) => {
  try {
    ...
    var data = await OrganizationLeaveTypeModel.findAll({
      where: Sequelize.and(Sequelize.or({ name: { [searchCondition]: search } }, { code: { [searchCondition]: search } }), { organization_id: args.data.organization_id }, { deleted_at: null }),
    });
    return data;
  } catch (error) {
    logger.error(error);
