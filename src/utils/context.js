...
const getUser = async (req, res, UserModel) => {
  ...
  if (!token) {
    ...
    // throw getMessage('NOT_LOGGEDIN');
    throw new ApolloError(getMessage('NOT_LOGGEDIN'));
  }

  if (!token.startsWith('Bearer ')) {
    // throw new AuthenticationError(getMessage('INVALID_TOKEN'));
    throw new ApolloError(getMessage('INVALID_TOKEN'));
    // throw res.status(401).json(getMessage('INVALID_TOKEN'));
  }
  ...
  try {
    ...
    // throw error;
    throw error;
    // throw res.sINVALID_TOKENstatus(401).json(error);
  }
};
